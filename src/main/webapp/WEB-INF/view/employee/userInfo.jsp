...
<body>
	...
	<div class="wrap">
		...
		<form action="<%=request.getContextPath()%>/employee/userInfo.do"
			method="post">
					...
					<table class="info-table">
						<tr>
							...
							<td><input type="text" name="companyName"
								value="${info.companyName}"></td>
							<th style="width: 25%;"><span class="req">*</span>代表者役職/代表者</th>
							<td>
								<!-- DB 연동을 위해 JSTL 조건문의 '대표이사'는 유지했습니다 --> <input type="text"
								name="repTitle"
								value="${info.repTitle != null ? info.repTitle : '대표이사'}"
								style="width: 60px;"> / <input type="text"
								name="repName" value="${info.repName}" style="width: 80px;">
							</td>
						</tr>
						<tr>
							<th><span class="req">*</span>事業者番号</th>
							<td><input type="text" name="businessNumber"
								value="${info.businessNumber}"></td>
							<th>法人登録番号</th>
							<td><input type="text" name="corpNumber"
								value="${info.corpNumber}"></td>
						</tr>
						<tr>
							<th>設立日</th>
							<td><input type="text" name="establishmentDate"
								value="${info.establishmentDate}" placeholder="YYYY-MM-DD"></td>
							<th>ホームページ</th>
							<td><input type="text" name="website"
								value="${info.website}"></td>
						</tr>
						<tr>
							<th><span class="req">*</span>事業所住所</th>
							<td colspan="3"><input type="text" name="zipCode"
								value="${info.zipCode}" style="width: 60px;">
								<button type="button" class="btn-gray">郵便番号</button> <input
								type="text" name="officeAddress" value="${info.officeAddress}"
						...
						<tr>
							...
							</select> - <input type="text" name="phone2" value="${info.phone2}"
								style="width: 40px;"> - <input type="text" name="phone3"
								value="${info.phone3}" style="width: 40px;"></td>
							<th>FAX番号</th>
							<td><select name="fax1">
									<option value="">代表(なし)</option>
							...
							</select> - <input type="text" name="fax2" value="${info.fax2}"
								style="width: 40px;"> - <input type="text" name="fax3"
								value="${info.fax3}" style="width: 40px;"></td>
						</tr>
						<tr>
							<th>業態</th>
							<td><input type="text" name="bizType"
								value="${info.bizType}"></td>
							<th>種目</th>
							<td><input type="text" name="bizItem"
								value="${info.bizItem}"></td>
						</tr>
					</table>
				</div>
					...
					<table class="info-table">
						<tr>
							...
							<td><input type="text" name="contactName"
								value="${info.contactName}" style="width: 90%;"></td>
						</tr>
						<tr>
							<th>部署</th>
							<td><select name="departmentId" style="width: 60%;">
									<option value="">選択</option>
									<c:forEach var="dept" items="${deptList}">
										<option value="${dept.id}"
											${info.departmentId == dept.id ? 'selected' : ''}>${dept.name}</option>
									</c:forEach>
							</select>
								<button type="button" class="btn-manage"
									onclick="openModal('deptModal')">管理</button></td>
						</tr>
						<tr>
							<th>役職</th>
							<td><select name="positionId" style="width: 60%;">
									<option value="">選択</option>
									<c:forEach var="pos" items="${posList}">
										<option value="${pos.id}"
											${info.positionId == pos.id ? 'selected' : ''}>${pos.name}</option>
									</c:forEach>
							</select>
								<button type="button" class="btn-manage"
									onclick="openModal('posModal')">管理</button></td>
						</tr>
						<tr>
							<th>電話番号</th>
							<td><select name="cPhone1">
									<option value="">選択</option>
									<option value="010" ${info.cPhone1 == '010' ? 'selected' : ''}>携帯電話(010)</option>
									<option value="050" ${info.cPhone1 == '050' ? 'selected' : ''}>インターネット(050)</option>
									<option value="0507"
										${info.cPhone1 == '0507' ? 'selected' : ''}>インターネット(0507)</option>
									<option value="070" ${info.cPhone1 == '070' ? 'selected' : ''}>インターネット(070)</option>
									<option value="0303"
										${info.cPhone1 == '0303' ? 'selected' : ''}>インターネット(0303)</option>
									<option value="0504"
										${info.cPhone1 == '0504' ? 'selected' : ''}>インターネット(0504)</option>
							...
							</select> - <input type="text" name="cPhone2" value="${info.cPhone2}"
								style="width: 40px;"> - <input type="text"
								name="cPhone3" value="${info.cPhone3}" style="width: 40px;">
							</td>
						</tr>
						<tr>
							<th>携帯電話番号</th>
							...
							</select> - <input type="text" name="mobile2" value="${info.mobile2}"
								style="width: 40px;"> - <input type="text"
								name="mobile3" value="${info.mobile3}" style="width: 40px;">
							</td>
						</tr>
						<tr>
							<th>メールアドレス</th>
			...
			<table class="info-table">
				<tr>
					...
					<td style="width: 35%;"><select name="calc1MonthType"
						style="padding: 4px;">
							<option value="">選択</option>
							<option value="P"
								${account != null && account.calc1MonthType == 'P' ? 'selected' : ''}>前月</option>
							<option value="C"
								${account == null || account.calc1MonthType == 'C' ? 'selected' : ''}>当月</option>
					</select> <select name="salaryCalc1" style="padding: 4px;">
							<option value="">選択</option>
							<c:forEach var="day" begin="1" end="31">
								<option value="${day}"
									${account.salaryCalculation1 == day ? 'selected' : ''}>${day < 10 ? '0' : ''}${day}日</option>
							</c:forEach>
							<option value="0"
								${account.salaryCalculation1 != null && account.salaryCalculation1 == 0 ? 'selected' : ''}>末日</option>
					</select> <span style="margin: 0 10px;">~</span> <select
						name="calc2MonthType" style="padding: 4px;">
							<option value="">選択</option>
							<option value="P"
								${account != null && account.calc2MonthType == 'P' ? 'selected' : ''}>前月</option>
							<option value="C"
								${account == null || account.calc2MonthType == 'C' ? 'selected' : ''}>当月</option>
					</select> <select name="salaryCalc2" style="padding: 4px;">
							<option value="">選択</option>
							<c:forEach var="day" begin="1" end="31">
								<option value="${day}"
									${account.salaryCalculation2 == day ? 'selected' : ''}>${day < 10 ? '0' : ''}${day}日</option>
							</c:forEach>
							<option value="0"
								${account.salaryCalculation2 != null && account.salaryCalculation2 == 0 ? 'selected' : ''}>末日</option>
					</select></td>

					<th style="width: 15%;">給与支給日</th>
					<td style="width: 35%;" colspan="3"><select
						name="paymentMonthType" style="padding: 4px;">
							<option value="C"
								${account == null || account.paymentMonthType == 'C' ? 'selected' : ''}>当月</option>
							<option value="N"
								${account != null && account.paymentMonthType == 'N' ? 'selected' : ''}>翌月</option>
					</select> <select name="salaryPaymentDate" style="padding: 4px;">
							<option value="">選択</option>
							<c:forEach var="day" begin="1" end="31">
								<option value="${day}"
									${account.salaryPaymentDate == day ? 'selected' : ''}>${day < 10 ? '0' : ''}${day}日</option>
							</c:forEach>
							<option value="0"
								${account.salaryPaymentDate != null && account.salaryPaymentDate == 0 ? 'selected' : ''}>末日</option>
					</select></td>
				</tr>
				<tr>
					<th>金融機関</th>
					<td><select name="bankName" style="padding: 4px;">
							<option value="">選択してください</option>
							<option value="국민은행"
								${account.bankName == '국민은행' ? 'selected' : ''}>国民銀行</option>
							<option value="기업은행"
								${account.bankName == '기업은행' ? 'selected' : ''}>企業銀行</option>
							<option value="농협은행"
								${account.bankName == '농협은행' ? 'selected' : ''}>農協銀行</option>
							<option value="신한은행"
								${account.bankName == '신한은행' ? 'selected' : ''}>新韓銀行</option>
							<option value="우리은행"
								${account.bankName == '우리은행' ? 'selected' : ''}>ウリィ銀行</option>
							<option value="하나은행"
								${account.bankName == '하나은행' ? 'selected' : ''}>ハナ銀行</option>
					</select></td>
					<th>口座番号</th>
					<td><input type="text" name="accountNumber"
						value="${account.accountNumber}" style="width: 80%;"></td>
					<th>口座名義人</th>
					<td><input type="text" name="depositStocks"
						value="${account.depositStocks}"></td>
				</tr>
				<tr>
					<th>給与振込バンキング</th>
					<td colspan="5">
						...
						<div
							style="background: #fff9e6; padding: 10px; border: 1px solid #ffeeba; display: inline-block;">
							<!-- kbId, kbPw는 DB 구조에 없으므로 기존대로 info 바인딩 유지 -->
							企業バンキング ID <input type="text" name="kbId" value="${info.kbId}">
							Password <input type="password" name="kbPw" value="${info.kbPw}">
							<button type="button" class="btn-manage"
								style="margin-left: 10px;">すぐにERP連携</button>
						</div>
					</td>
				</tr>
			</table>
			...
			<div class="img-box-wrap">
				<div>
					...
					<div class="img-box">
						...
						<div class="img-desc">
							ロゴは横幅150pxのサムネイルで生成されます。<br>透明なPNG画像の使用を推奨します。<br>
							<br>
							<button type="button" class="btn-manage">登録</button>
				...
				<div>
					...
					<div class="img-box">
						<div class="img-placeholder">NO IMAGE</div>
						<div class="img-desc">
							横幅150pxのサムネイル、PNGファイルを推奨します。<br>無料印鑑提供：stamp.yesform.com<br>
							<br>
							<button type="button" class="btn-manage">登録</button>
							<button type="button" class="btn-gray">削除</button>
						</div>
	...
	<form id="manageForm" method="post"
		action="<%=request.getContextPath()%>/employee/manageDeptPos.do"
		style="display: none;">
		<input type="hidden" name="type" id="mType"> <input
			type="hidden" name="action" id="mAction"> <input
			type="hidden" name="id" id="mId"> <input type="hidden"
			name="name" id="mName">
	</form>
	...
	<div id="deptModal" class="modal-bg">
		<div class="modal-content">
			...
			<div class="item-list">
				<c:forEach var="dept" items="${deptList}">
					<div class="item-row">
						<span>${dept.name}</span><span><a
							onclick="manageItem('dept', 'edit', ${dept.id}, '${dept.name}')">修正</a>
							| <a
							onclick="manageItem('dept', 'delete', ${dept.id}, '${dept.name}')"
							style="color: #e74a3b;">削除</a></span>
					</div>
				</c:forEach>
				<div class="add-input-row" id="deptAddRow">
					<input type="text" id="deptNewName" placeholder="新しい部署名"><span><a
						class="save-link" onclick="saveNewItem('dept')">保存</a> | <a
						class="cancel-link" onclick="hideAddRow('dept')">キャンセル</a></span>
				</div>
				<div class="add-btn" id="deptAddBtn" onclick="showAddRow('dept')">+
					追加する</div>
			</div>
	...
	<div id="posModal" class="modal-bg">
		<div class="modal-content">
			...
			<div class="item-list">
				<c:forEach var="pos" items="${posList}">
					<div class="item-row">
						<span>${pos.name}</span><span><a
							onclick="manageItem('pos', 'edit', ${pos.id}, '${pos.name}')">修正</a>
							| <a
							onclick="manageItem('pos', 'delete', ${pos.id}, '${pos.name}')"
							style="color: #e74a3b;">削除</a></span>
					</div>
				</c:forEach>
				<div class="add-input-row" id="posAddRow">
					<input type="text" id="posNewName" placeholder="新しい役職名"><span><a
						class="save-link" onclick="saveNewItem('pos')">保存</a> | <a
						class="cancel-link" onclick="hideAddRow('pos')">キャンセル</a></span>
				</div>
				<div class="add-btn" id="posAddBtn" onclick="showAddRow('pos')">+
					追加する</div>
			</div>
