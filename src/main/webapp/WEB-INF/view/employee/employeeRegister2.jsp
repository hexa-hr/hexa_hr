...
/* 🌟 변경 포인트: 버튼을 얇게 만들기 위해 height를 35px로, padding을 줄였습니다. */
...
<body>
	...
	<div class="wrap">
		<div class="sidebar">
			<div style="background: white; padding: 15px; border: 1px solid #ccc; text-align: center; margin-bottom: 20px;">
				...
				<p style="margin: 10px 0 0 0; font-weight: bold; font-size: 14px;">社員番号: ${employeeId}</p>
			</div>
			...
			<h3>社員情報 2</h3>
			<div class="menu-grid">
				<a href="#cert" class="menu-btn">資格・免許</a> <a href="#training" class="menu-btn">教育訓練</a> <a href="#reward" class="menu-btn">賞罰</a>
				<a href="#appointment" class="menu-btn">発令</a> <a href="#referrer" class="menu-btn">推薦・身元保証</a> <a href="#retirement" class="menu-btn">退職</a>
			</div>
		</div>
		...
		<div class="container">
			...
			<form action="<%=request.getContextPath()%>/employee/register2_process.do" method="post" target="hidden_iframe">
					...
					<c:if test="${not empty langList}">
						<c:forEach var="lang" items="${langList}">
							<tr>
								...
								<td><select name="langReading"><option value="상" ${lang.readingAbility == '상' ? 'selected' : ''}>上</option><option value="중" ${lang.readingAbility == '중' ? 'selected' : ''}>中</option><option value="하" ${lang.readingAbility == '하' ? 'selected' : ''}>下</option></select></td>
								<td><select name="langWriting"><option value="상" ${lang.writingAbility == '상' ? 'selected' : ''}>上</option><option value="중" ${lang.writingAbility == '중' ? 'selected' : ''}>中</option><option value="하" ${lang.writingAbility == '하' ? 'selected' : ''}>下</option></select></td>
								<td><select name="langSpeaking"><option value="상" ${lang.speakingAbility == '상' ? 'selected' : ''}>上</option><option value="중" ${lang.speakingAbility == '중' ? 'selected' : ''}>中</option><option value="하" ${lang.speakingAbility == '하' ? 'selected' : ''}>下</option></select></td>
							</tr>
						</c:forEach>
					</c:if>
				</table>
				...
				<!-- 2. 교육 훈련 -->
				<div class="section-title" id="training">教育訓練<button type="button" class="add-btn" onclick="addRow('trainingTable')">+ 追加</button></div>
				<table id="trainingTable">
					<tr><th>教育区分</th><th>教育名</th><th>開始日</th><th>終了日</th><th>教育機関</th><th>教育費</th><th>還付金</th><th>削除</th></tr>
				...
				<!-- 3. 상벌 -->
				<div class="section-title" id="reward">賞罰<button type="button" class="add-btn" onclick="addRow('rewardTable')">+ 追加</button></div>
				<table id="rewardTable">
					<tr><th>区分</th><th>賞罰名</th><th>賞罰権者</th><th>日付</th><th>内容</th><th>備考</th><th>削除</th></tr>
				...
				<!-- 4. 발령 -->
				<div class="section-title" id="appointment">発令<button type="button" class="add-btn" onclick="addRow('appointmentTable')">+ 追加</button></div>
				<table id="appointmentTable">
					<tr><th>発令区分</th><th>日付</th><th>部署</th><th>役職</th><th>職責</th><th>備考</th><th>削除</th></tr>
				...
				<!-- 5. 추천 & 신원보증 -->
				<div class="section-title" id="referrer">推薦 & 身元保証 (推薦人)<button type="button" class="add-btn" onclick="addRow('referrerTable')">+ 追加</button></div>
				<table id="referrerTable">
					<tr><th>氏名</th><th>続柄</th><th>会社名</th><th>役職</th><th>電話番号</th><th>削除</th></tr>
				...
				<div class="section-title" style="margin-top: 10px;">身元保証 (保証人)<button type="button" class="add-btn" onclick="addRow('guarantorTable')">+ 追加</button></div>
				<table id="guarantorTable">
					<tr><th>氏名</th><th>続柄</th><th>住民登録番号</th><th>保証金額</th><th>連絡先</th><th>削除</th></tr>
				...
				<table>
					<tr>
						...
						<td><select name="retirementType" style="width: 80%;">
								<option value="">選択</option>
								<option value="정년퇴직" ${retirement.retirementType == '정년퇴직' ? 'selected' : ''}>定年退職</option>
								<option value="자진퇴사" ${retirement.retirementType == '자진퇴사' ? 'selected' : ''}>自己都合退社</option>
								<option value="권고사직" ${retirement.retirementType == '권고사직' ? 'selected' : ''}>勧奨退職</option>
						</select></td>
						<th>退職日</th>
				...
				<div style="text-align: center; margin-top: 50px; margin-bottom: 50px;">
					<button type="submit" style="padding: 12px 40px; background-color: #4e73df; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 16px; font-weight: bold;">付加情報保存</button>
					<button type="reset" style="padding: 12px 40px; background-color: #a5a5a5; color: white; border: none; border-radius: 5px; cursor: pointer; margin-left: 10px; font-size: 16px; font-weight: bold;">初期化</button>
				</div>
			</form>
		</div>
	</div>
	...
	<script>
		...
		window.onload = function() {
            ...
            // URL에서 tab 파라미터를 읽어 해당 위치로 부드럽게 자동 스크롤 이동
		...
		function addRow(tableId) {
			...
			if (tableId === 'certTable') {
				...
				html = '<td><input type="text" name="guaName"></td><td><input type="text" name="guaRelation"></td><td><input type="text" name="guaRrn" placeholder="[住民登録番号入力]"></td><td><input type="number" name="guaAmount"></td><td><input type="text" name="guaPeriod" placeholder="連絡先"></td>';
			}
