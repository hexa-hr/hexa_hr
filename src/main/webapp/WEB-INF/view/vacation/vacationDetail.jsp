...
<body>
	...
	<div class="modal-container">
		<!-- 상단 타이틀 -->
		<div class="title-area">[${not empty detailList[0].departmentName ? detailList[0].departmentName : '부서미지정'}]
			${not empty detailList[0].koreanName ? detailList[0].koreanName : '사원'}
			사원 휴가현황</div>
		...
		<table class="detail-table">
			...
			<tbody>
				<c:if test="${empty detailList}">
					<tr>
						<td colspan="7" style="padding: 30px; color: #888;">조회된 휴가 상세
							내역이 없습니다.</td>
					</tr>
				</c:if>
		...
		<c:set var="totalDays"
			value="${empty detailList ? 15 : detailList[0].totalDays}" />
		<%-- 2. 잔여일수 계산 (총 휴가일수 - 사용일수) --%>
		<c:set var="remainingDays" value="${totalDays - totalUsed}" />

		<!-- 하단 합계 바 -->
		<div class="summary-bar">
			<span>합 계</span> <span> • 총 휴가일수 : <fmt:formatNumber
					value="${totalDays}" pattern="#,##0.000" />&nbsp;&nbsp;&nbsp;&nbsp;
				• 사용일수 : <span style="color: #2563eb;"><fmt:formatNumber
						value="${totalUsed}" pattern="#,##0.000" /></span>&nbsp;&nbsp;&nbsp;&nbsp;
				• 잔여일수 : <span style="color: #e11d48;"><fmt:formatNumber
						value="${remainingDays}" pattern="#,##0.000" /></span>
			</span>
		</div>
</body>
</html>