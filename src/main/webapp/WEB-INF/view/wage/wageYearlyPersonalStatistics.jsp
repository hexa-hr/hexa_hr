...
<body>
	...
	<div class="wrap">
		<div class="container">
			...
			<p class="page-desc">帰属年度と社員を選択すると、該当社員の直近10年間の年俸状況を確認できます。</p>
			...
			<c:if test="${not empty yearlyPersonalStatistics}">
				...
				<div class="table-container">
					<table class="data-table">
						...
						<tbody>
							<tr>
								<td class="row-title">年俸額（千ウォン）</td>
								<c:forEach var="row" items="${yearlyPersonalStatistics.rows}">
									<td>
										<c:choose>
											<c:when test="${not row.hasData}">-</c:when>
											<c:otherwise><fmt:formatNumber value="${row.annualSalary / 1000}" pattern="#,##0" /></c:otherwise>
										</c:choose>
									</td>
								</c:forEach>
							</tr>
									...
									<td>
										<c:choose>
											...
											<c:when test="${row.salaryGrowthRate > 0}">
												<span class="text-red"> <fmt:formatNumber value="${row.salaryGrowthRate}" pattern="0.0" />%</span>
											</c:when>
											<c:when test="${row.salaryGrowthRate < 0}">
												<span class="text-blue"> <fmt:formatNumber value="${row.salaryGrowthRate}" pattern="0.0" />%</span>
											</c:when>
											<c:otherwise>
												<span class="text-neutral"> <fmt:formatNumber value="${row.salaryGrowthRate}" pattern="0.0" />%</span>
											</c:otherwise>
										</c:choose>
									</td>
								</c:forEach>
							...
							<tr>
								<td class="row-title">控除額（千ウォン）</td>
								<c:forEach var="row" items="${yearlyPersonalStatistics.rows}">
									<td>
										<c:choose>
											<c:when test="${not row.hasData}">-</c:when>
											<c:otherwise><fmt:formatNumber value="${row.totalDeduction / 1000}" pattern="#,##0" /></c:otherwise>
										</c:choose>
									</td>
								</c:forEach>
							</tr>
							...
							<tr>
								<td class="row-title">差引支給額（千ウォン）</td>
								<c:forEach var="row" items="${yearlyPersonalStatistics.rows}">
									<td>
										<c:choose>
											<c:when test="${not row.hasData}">-</c:when>
											<c:otherwise><fmt:formatNumber value="${row.netPayment / 1000}" pattern="#,##0" /></c:otherwise>
										</c:choose>
									</td>
								</c:forEach>
							</tr>
	...
	<script>
		const yearLabels = [
			<c:forEach var="row" items="${yearlyPersonalStatistics.rows}" varStatus="status">
				"${row.year}年"<c:if test="${!status.last}">,</c:if>
			</c:forEach>
		];
		...
		const personalChartCanvas = document.getElementById("yearlyPersonalChart");
				...
				options : {
						...
						tooltip : {
							...
							callbacks : {
								...
								beforeBody : function(items) {
									...
									return "年俸額（千ウォン）: " + formatThousandWon(salary);
								},
					...
					scales : {
						...
						y : {
							...
							beginAtZero : true,
							grid : { color: "#eee" },
							title : { display : true, text : "年俸額（千ウォン）", color : axisColor, font: {weight: 'bold'} },
							ticks : {
								color : axisColor,
